...
> This is the api12.3 branch: version 2.1 for HFS 3.2

Requires HFS with plugin API 12.3 or newer. On HFS 3.3 and newer (API 13.4) logins are refused through
HFS official finalizingLogin veto, and HFS words the refusal as "UnauthorizedError: Invalid OTP".
On older HFS, where plugins can't refuse a login, the plugin makes HFS's own login step fail instead.
