...
exports.init = async api => {
    ...
    const CAN_VETO = api.Const?.API_VERSION >= 13.4 // since HFS 3.3, finalizingLogin refuses the login if a listener returns a message
    ...
    function denyLogin(ctx, message, hard, mark) {
        ...
        if (CAN_VETO)
            return message
        // older HFS ignore what listeners return, and the exceptions they throw. setLoggedIn() proceeds assigning
        // session.username: we make that assignment throw, so the login stops before the session is saved and before
        // the login event, and hfs answers 401 with our message
